...
impl<'r, 'l, L: SubstateStore> Process<'r, 'l, L> {
    ...
    fn move_lazy_map_into_component(&mut self, unclaimed_lazy_map: UnclaimedLazyMap, component_address: Address, mid: Mid) {
    ...
    fn handle_create_component(
        ...
        for mid in data.lazy_maps {
            ...
            self.move_lazy_map_into_component(unclaimed_lazy_map, component_address, mid);
        }
    ...
    fn handle_put_lazy_map_entry(
            ...
            if !old_entry_vids.remove(&vid) {
                ...
                match lazy_map_state {
                    Unclaimed(ancestor_mid) => {
                        let unclaimed_lazy_map = self.unclaimed_lazy_maps.get_mut(&ancestor_mid).unwrap();
        ...
        for mid in new_entry_state.lazy_maps {
            if !old_entry_mids.remove(&mid) {
                ...
                match lazy_map_state {
                    // Move/merge unclaimed lazy map into parent
                    Unclaimed(ancestor_mid) => {
                        let parent_lazy_map =
                            self.unclaimed_lazy_maps.get_mut(&ancestor_mid).unwrap();
                        parent_lazy_map.child_lazy_maps.insert(mid, child_lazy_map.lazy_map);

                        for (mid, lazy_map) in child_lazy_map.child_lazy_maps {
                            parent_lazy_map.child_lazy_maps.insert(mid, lazy_map);
                        }
                        for (vid, vault) in child_lazy_map.child_vaults {
                            parent_lazy_map.child_vaults.insert(vid, vault);
                        }
                    }
                    PartOfComponent(component_address) => {
                        self.move_lazy_map_into_component(child_lazy_map, component_address, mid);
                    }
                }
            }
        }
